...
pub fn combine_tables(t1: c.Table, t2: c.Table) {
  ...
  let t1_yay =
    list.map(t1_columns, fn(col) {
      c.Column(
        t1.name <> "." <> col.name,  // Use dot separator for clarity
        col.type_,
        col.is_primary,
        col.is_nullable,
        col.is_unique,
  ...
  let t2_yay =
    list.map(t2_columns, fn(col) {
      c.Column(
        t2.name <> "." <> col.name,  // Use dot separator for clarity
        col.type_,
        col.is_primary,
        col.is_nullable,
        col.is_unique,
  ...
  c.Table(t1.name <> "_" <> t2.name, list.append(t1_yay, t2_yay))  // Keep underscore for table name
}
